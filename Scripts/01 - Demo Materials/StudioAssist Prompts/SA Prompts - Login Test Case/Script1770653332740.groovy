...
/*DEMO Item #1:Generate Test case from TestOps foundation
 
 Automate the flow: TC-17997970 from the "Katalian Banking 2.0" TestOps Project using the below parameters:
1. Connect to TestOps project ""Katalian Banking 2.0" and locate test case TC-17997970
2. Open a browser instance, take the page source for each test step page
3. Reuse objects that are alredy found in the Object Repository
4. Build all new objects required for the test script into Object Repository folder  /AI Demo/Req/Login/"
5. Build the test case using the above objects in the folder specified in step 1
6. Use built-in WebUI keywords only and add all relevant includes
7. Store screenshots automatically in the default Katalon Reports folder
8. Do NOT encrypt the entered password
9. Save the test case to folder "/AI Demo/Req/Login/" in Katalon Studio
10. Close browser instance after test script has been created

 */

/* Generate Test Case - Prompt based instructions
Objective:
Generate and save a runnable Katalon Studio WebUI test case to validate successful login for the Katalian Banking demo application.
